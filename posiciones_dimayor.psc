Algoritmo posiciones_dimayor
	...
	Para i <- 0 Hasta equipos-1 Hacer
		Escribir "Ingresa el nombre del equipo ", i+1
		Leer nombre[i]
		Escribir "Ingresa la cantidad de partidos jugados"
		Leer PaJ[I]
		Escribir "¿Cuántos partidos ganó?" 
		Leer G[i]
		Escribir "¿Cuántos partidos empató?"
		Leer E[i]
		Escribir "¿Cuántos partidos perdió?"
		Leer P[i]
		Si PaJ[i] < G[i]+E[i]+P[i] Entonces
			Escribir "La tabla se imprimirá con errores"
		FinSi
		Escribir"¿Cuántos goles a favor realizó?"
		Leer GF[i] 
		Escribir "¿Cuántos goles recibió?"
		Leer GC[i] 
		DG[i] <- GF[i] - GC[i]
		Pts[i] <- (G[i]*3) + (E[i]*1)
	FinPara
